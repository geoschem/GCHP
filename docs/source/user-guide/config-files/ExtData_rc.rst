...
.. describe:: Ext_AllowExtrap
   ...
   and the entries of :ref:`cfg-hco-cfg` which do this are ignored
   in GCHP.
...
.. describe:: Name
   ...
   information in :file:`ExtData.rc`. All file I/O information in
...
.. describe:: Clim
   ...
   Whether the data is climatology. Enter :literal:`Y` if the data is a
   12 month climatology, enter year if the data is daily climatology
   (i.e. :literal:`2019`), :literal:`D` if the file is monthly day-of-week
   scale factors (7 values for each of 12 months), or :literal:`N` for
...
.. describe:: Conservative
   ...
   :literal:`Y` to use mass conserving regridding, :literal:`F;{VALUE}`
   for fractional regridding, or :literal:`N` to use
...
.. describe:: Refresh
   ...
   * :literal:`-` : Update variable data only once. Use this if the data
     is constant in time. |br|
     |br|

   * :literal:`0` : Update variable data at every timestep using linear
   ...
   * :literal:`0:003000` (or other HHMMSS specification for hours,
   ...
   * :literal:`F0:003000` (or other HHMMSS specification for hours,
   ...
   * :literal:`%y4-%m2-%h2T%h2:%n2:00` (or other combination of time
     ...
     :literal:`%y4-%m2-%d2T12:00:00` changes at the start of each day
     because that is when the evaluation of :literal:`%y4-%m2-%d2`
   ...
   * :literal:`F%y4-%m2-%h2T%h2:%n2:00` (or other combination of time
     tokens) : Like the previous option except that there is no time
     interpolation.

.. describe:: Offset Factor

   Value the data will be shifted by upon read. Use :literal:`none` for
   no shifting.

.. describe:: Scale Factor

   Value the data will be scaled by upon read. This is useful if you
   want to convert units upon read, such as from :literal:`Pa` to
   :literal:`hPa`. Use :literal:`none` for no scaling.

.. describe:: External File Variable

   Name of the variable to read in the netCDF data file.

.. describe:: External File Template

   Path to the netCDF data file, including time tokens as needed
   (:literal:`%y4` for year, :literal:`%m2` for month, :literal:`%d2` for
   day, :literal:`%h2` for hour, :literal:`%n2` for minutes). If there are no
...
.. describe:: Reference Time and Period
   ...
   **OPTIONAL**. Period of data with reference time. This optional
   ...
   :literal:`2000-01-01T01:30:00P03:00`. The first part (before
   :literal:`P`) is the reference date (must be on or before your
   simulation start), and the second part (after :literal:`P`) is the
   ...
   the file template contains the :literal:`n2` time token.
